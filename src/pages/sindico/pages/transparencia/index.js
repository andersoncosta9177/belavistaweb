...
function Transparencia() {
  ...
  const openEditModal = (gasto) => {
    try {
      const [day, month, year] = gasto.data.split("/");
      const date = new Date(year, month - 1, day);

      setCurrentGasto(gasto);
      setSelectedDate(date);
      setFormData({
        servico: gasto.servico,
        valor: gasto.valor,
  ...
  const validateForm = () => {
    if (!formData.servico.trim()) {
      Alert.alert("Aviso", "Informe o serviço realizado");
      return false;
    }
    if (!formData.valor.trim()) {
      Alert.alert("Aviso", "Informe o valor");
      return false;
    }
    if (!formData.empresa.trim()) {
      Alert.alert("Aviso", "Informe a empresa");
      return false;
    }
    return true;
  };
  ...
  const saveGasto = async () => {
    ...
    try {
      ...
      if (currentGasto) {
        await update(
          ref(db, `${TRANSPARENCIA_COLLECTION}/${currentGasto.id}`),
          gastoData
        );
        setGastos(
          gastos.map((g) =>
            g.id === currentGasto.id ? { ...g, ...gastoData } : g
          )
        );
        Alert.alert("Sucesso", "Gasto atualizado!");
      } else {
        const docRef = push(ref(db, TRANSPARENCIA_COLLECTION));
        await set(docRef, gastoData);
        setGastos([{ id: docRef.key, ...gastoData }, ...gastos]);
        Alert.alert("Sucesso", "Gasto adicionado!");
      }
      setModalVisible(false);
    } catch (error) {
      console.error("Erro ao salvar:", error);
      Alert.alert("Erro", "Falha ao salvar gasto");
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteGasto = async (id) => {
    try {
      setLoading(true);
      await remove(ref(db, `${TRANSPARENCIA_COLLECTION}/${id}`));
      setGastos(gastos.filter((g) => g.id !== id));
      Alert.alert("Sucesso", "Gasto removido!");
    } catch (error) {
      console.error("Erro ao deletar:", error);
      Alert.alert("Erro", "Falha ao remover gasto");
    } finally {
      setLoading(false);
    }
  };
            ...
            <Card key={gasto.id} containerStyle={styles.card}>
              ...
              <View style={styles.cardFooter}>
                <Button
                  ...
                  onPress={() => openEditModal(gasto)}
                />
                <Button
                  buttonStyle={styles.deleteButton}
                  icon={{ name: "delete", type: "material", color: "white" }}
                  onPress={() => {
                    Alert.alert("Confirmar", "Deseja excluir este gasto?", [
                      { text: "Cancelar", style: "cancel" },
                      { text: "Excluir", onPress: () => deleteGasto(gasto.id) },
                    ]);
                  }}
                />
              </View>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <Text style={styles.modalTitle}>
              {currentGasto ? "Editar Gasto" : "Novo Gasto"}
            </Text>
            ...
            <TextInput
              ...
              onChangeText={(text) =>
                setFormData({ ...formData, servico: text })
              }
