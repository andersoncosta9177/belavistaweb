...
function VagasDisponiveisMoradores({ navigation }) {
  ...
  const deletarVaga = async (vagaId) => {
    try {
      Alert.alert(
        ...
        [
          ...
          { 
            text: "Remover", 
            onPress: async () => {
              const vagaRef = ref(db, `DadosBelaVista/administracao/vagasGaragem/${vagaId}`);
              await set(vagaRef, null);
              Alert.alert("Sucesso", "Vaga removida com sucesso!");
            }
          }
        ]
      );
      ...
      Alert.alert("Erro", "Não foi possível remover a vaga");
    }
  };
  ...
  return (
    <GradientLayout style={styles.container}>
          ...
          vagas.map((vaga) => (
            <Card key={vaga.id} containerStyle={styles.card}>
              ...
              <View style={styles.infoContainer}>
                ...
                <View style={styles.infoRow}>
                  <Icon name="home" type="font-awesome" size={14} color="#6c757d" />
                  <Text style={styles.infoText}>Apartamento: {vaga.apartamento || 'Condomínio'}</Text>
                </View>
                <View style={styles.infoRow}>
              ...
              <TouchableOpacity 
                ...
                onPress={() => openWhatsApp(vaga.telefone)}
                activeOpacity={0.7}
              >
              ...
              {userId === vaga.uidAnunciante && (
                <TouchableOpacity 
                  style={styles.deleteButton}
                  onPress={() => deletarVaga(vaga.id)}
                >
                  <Icon name="trash" type="font-awesome" size={20} color="#FFF" />
                  <Text style={styles.deleteButtonText}>Remover Anúncio</Text>
                </TouchableOpacity>
              )}
            </Card>
          ))
      ...
      <Link asChild href="/src/pages/moradores/pages/garagem/anunciar">
        <TouchableOpacity style={styles.addButton}>
          <Icon name="plus" type="font-awesome-5" size={20} color="#FFF" />
          <Text style={styles.addButtonText}>Anunciar Vaga</Text>
        </TouchableOpacity>
      </Link>
    </GradientLayout>
  );
...
export default VagasDisponiveisMoradores;